...
mapfile -t csv_files < <(find . -maxdepth 1 -name "*.csv" -print0 | xargs -0 -n1 basename)
mapfile -t cbm_files < <(find . -maxdepth 1 -name "*.cbm" -print0 | xargs -0 -n1 basename)
...
docker_image_name="autocatboost"

# Function to run Docker container for training
run_training() {
    local_csv_file=$1
    echo "Running training with $local_csv_file"
    docker run -v "$(pwd)":/data $docker_image_name --train --file /data/"$local_csv_file"
}
...
run_prediction() {
    ...
    docker run -v "$(pwd)":/data $docker_image_name --predict --file /data/"$local_csv_file" --model /data/"$local_cbm_file"
}
